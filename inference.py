...
def evaluate():
    ...
    if seq_to_seq_flag:
        path = os.getcwd()+'/trained_model/seq_to_seq/'
    else:
        path = os.getcwd()+'/trained_model/ctc/'        

    model = avg_models(path, 21, 50)        
    model.eval()
    ...
    for batch in data_loader: 
        ...
        for spec_ in batch[0]:
            ...
            if seq_to_seq_flag == True:
                out,scores,best_combined=beam_search(model, spec_.unsqueeze(0),  trg, max_length=max_utterance_length, beam_size=beam_size, return_best_beam=False, weight_ctc=w_ctc)
            else:
                best_combined = beam_predict(model, spec_.unsqueeze(0), words=words, max_length=max_utterance_length, weight_ctc = 0.5)
            ...
            if bpe_flag==True:
                ...
                if seq_to_seq_flag == True:
                    ...
                    print(set_," BEAM_OUT:",  apply_lex(re.sub(r"[#^$]+","",best_combined.lower()),words))
