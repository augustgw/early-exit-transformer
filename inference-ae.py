    ...
    for set_ in "test-clean","test-other":#"dev-clean", "dev-other":
        ...
        for batch in data_loader: 
            ...
            for spec_, v_l in zip(batch[0],valid_length):
                ...
                print("MAX-MIN:", max_len, min_len, spec_.size(1), trg_expect_.size() )
                for n in range(1,n_enc_replay+1):
                    encoder_output = model._encoder_(spec_.unsqueeze(0), v_l.unsqueeze(0), n).to(device)
                    ...
                    out,scores,best_combined=beam_search(model,encoder_output, n, spec_.unsqueeze(0), trg, valid_length, max_length=max_len, beam_size=beam_size, return_best_beam=True, weight_ctc=w_ctc)
                    del encoder_output
                    if bpe_flag==True:
                        print(set_," BEAM_OUT_",n,":",  apply_lex(sp.decode(best_combined).lower(),words))
                del spec_
                del v_l
            del batch
