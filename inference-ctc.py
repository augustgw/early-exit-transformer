            ...
            for enc in encoder:
                ...
                if infer_flag == True:
                    ...
                    for best_ in best_combined:
                        if bpe_flag == True:
                            print(set_, "[", entropy.item(), "][", prob.item(
                            ), "] BEAM_OUT_", i, ":",  apply_lex(sp.decode(best_).lower(), words))
                        else:
                            print(set_, " BEAM_OUT_", i, ":",  apply_lex(
                                re.sub(r"[#^$]+", "", best_.lower()), words))
                else:
                    ...
                    for best_ in best_combined:
                        if bpe_flag == True:
                            ...
                            print(set_, "BEAM_OUT_", i, ":", apply_lex(
                                sp.decode(best_combined[0][0].tokens).lower(), words))
                        else:
                            print(set_, " BEAM_OUT_", i, ":",  apply_lex(
                                re.sub(r"[#^$]+", "", best_.lower()), words))
